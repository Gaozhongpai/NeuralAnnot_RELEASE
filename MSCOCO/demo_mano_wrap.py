...
def demo():
    ...
    for hand_type in ('right', 'left'):
        ...
        recover = apply_transformation(mesh_cam_trans, rotation_matrix, translation, is_inv=True)
        parameter_pca = hand_pca[hand_type].transform(mesh_cam_trans.view(1, -1))[:, :num_comps]
        
        # mesh render
        img = cv2.imread(img_path)
        img_2d = img.copy()
        cv2.circle(img_2d, (int(trans_uv[0, 0, 0]), int(trans_uv[0, 0, 1])), 3, (0,255,0), -1)
        cv2.circle(img_2d, (int(joint_2d[0, 0, 0]), int(joint_2d[0, 0, 1])), 3, (255,0,0), -1)
        # img_2d = draw_skeleton(img, joint_2d[0].numpy(), True)
        
        rendered_img = render_mesh(img, mesh_cam.cpu().numpy(), mano_layer[hand_type].faces, {'focal': focal, 'princpt': princpt})
        cv2.imwrite('mano_' + hand_type + '.jpg', rendered_img)
        cv2.imwrite('mano_' + hand_type + '_2d.jpg', img_2d)
